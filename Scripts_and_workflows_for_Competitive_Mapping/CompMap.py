#this python script is edited from an original provided by Santiago Sanchez-Ramirez
    ...
    for name in reads:
        ...
        try:
            ...
            try:
                ...
                mismatches2 = 0
                for x in name_indexed1.find(name):
                    if not x.is_unmapped or not x.is_secondary or not x.is_supplementary:
                        try:
                          score1 = x.get_tag('AS')
                        except KeyError:
                          score1 = -500
                ...
                for x in name_indexed2.find(name):
                    if not x.is_unmapped or not x.is_secondary or not x.is_supplementary:
                        ...
                        try :    
                          ...
                          mismatches2 = 500
                if (score1 == -500 and mismatches1 == 500) or  (score2 == -500 and mismatches2 == 500):
                  continue
                if (score1 == -500 and mismatches2 == 500) or  (score2 == -500 and mismatches1 == 500):
                  continue                                        
                if score1 > score2:
                    for x in name_indexed1.find(name):
                        out1.write(x)
